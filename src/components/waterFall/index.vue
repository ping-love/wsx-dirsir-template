...
<template>
  <div class="waterfallWrap">
    ...
    >
      ...
      >
        <img :src="item.src" :style="{ width: itemWidth / remBase + 'rem' }" alt="" />
      </div>
    </van-list>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      itemMarginRight: 10, // 右边距
      itemMarginBottom: 10, // 下边距
  ...
  methods: {
    ...
    imgPreloading() {
      for (let i = 0; i < this.currentList.length; i++) {
        let imgEl = new Image()
        imgEl.src = this.currentList[i]
        imgEl.onload = imgEl.onerror = e => {
          let imgData = {}
          //根据设定的列宽度求出图片的高度
          imgData.height = (this.itemWidth / imgEl.width) * imgEl.height
          imgData.src = this.currentList[i]
